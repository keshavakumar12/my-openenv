...
def grade_logic_fix(conn: sqlite3.Connection, agent_query: str, correct_query: str, difficulty: str = "medium") -> dict:
    ...
    if agent_rows == correct_rows:
        return {"score": 0.99, "reason": "Perfect — logic is correct, all rows match."}
    ...
    if len(correct_rows) > 0:
        ...
        if extra > 0:
            ...
            return {"score": clamp_score(score), "reason": f"Partial match: {len(overlap)}/{len(correct_rows)} correct rows, {extra} extra rows."}
 
        score = round(b["base"] + (overlap_ratio * b["scale"]), 2)
        score = min(score, b["near"])
        return {"score": clamp_score(score), "reason": f"Partial match: {len(overlap)}/{len(correct_rows)} correct rows."}
...
def grade_optimization(conn: sqlite3.Connection, agent_query: str, correct_query: str, broken_query: str) -> dict:
    ...
    Scoring:
      - 0.01 : agent query doesn't execute
      ...
      - 0.99 : correct results and significantly simpler/optimized
    """
    agent_result = execute_query(conn, agent_query)
    correct_result = execute_query(conn, correct_query)
    ...
    if agent_norm == correct_norm:
        return {"score": 0.99, "reason": "Perfect — query is optimized and matches the ideal solution."}
    ...
    if improved:
        return {"score": clamp_score(score), "reason": f"Good optimization — query is simpler (from {broken_subqueries + 1} to {agent_subqueries + 1} SELECT statements, length reduced by {round((1 - agent_len / broken_len) * 100)}%)."}
...
class SQLFixerEnvironment:
    ...
    def step(self, action: str) -> dict:
        ...
        if task_type == "syntax_fix":
            ...
            result = {"score": 0.01, "reason": f"Unknown task type: {task_type}"}
 
        
        done = (result["score"] >= 0.99) or (self.steps_taken >= self.max_steps)
...
if __name__ == "__main__":
    ...
    print(f"\n{'=' * 60}")
    print("Validating ALL tasks (correct_query must score 0.99)...")
    print(f"{'=' * 60}")
    failures = []
    for i, task in enumerate(TASKS):
        ...
        if task_type == "syntax_fix":
            ...
            result = {"score": 0.01, "reason": "unknown type"}
    ...
    print(f"\n{'=' * 60}")
    print("Verifying broken queries don't score 0.99...")
    print(f"{'=' * 60}")
    false_positives = []
    for i, task in enumerate(TASKS):
        schema_name = task.get("schema_name", "hr")
    ...
    if false_positives:
        print(f"\n  FALSE POSITIVES ({len(false_positives)}) — broken query scored 0.99:")
        for idx, desc, score in false_positives:
            print(f"    Task {idx}: {desc}...")
    else:
        print(f"\n  All {len(TASKS)} broken queries correctly score < 0.99!")
